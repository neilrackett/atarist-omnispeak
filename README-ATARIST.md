...
- Frame rate, measured in Hatari, and it depends heavily on the scene
  because the cost is dominated by how many tiles the refresh manager
  has to redraw:

  | Scene | blits/frame | Mega STE | Plain 8 MHz STE |
  | --- | --- | --- | --- |
  | World map | 4-6 | ~28 fps | ~21 fps |
  | In a level | 30-60 | ~9-12 fps | ~5-7 fps |

  The world-map figure is the engine's own ceiling rather than the
  hardware's: the refresh manager paces itself to a minimum of two
  70 Hz tics, capping it near 35 fps, and spends the rest of the frame
  waiting. In a level nothing is waiting, and roughly half the frame is
  pixel copying, so the blitters are the remaining target there. The
